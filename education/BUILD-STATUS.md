...
**AI grader (new, 2026-06-23):** Supabase Edge Function `grade-response` (deployed, ACTIVE, verify_jwt on) grades free-response answers against a model answer + key_points semantically (thesis match, not word-for-word) → `{score, feedback, matched, missing}`. Calls Anthropic; model via `GRADER_MODEL` env (default `claude-3-5-haiku-latest`). **REQUIRES the `ANTHROPIC_API_KEY` secret to be set on the project** (Dashboard → Project Settings → Edge Functions / Secrets) — until then written answers score 0 with a clear error. `auth.js` exposes `GEM.gradeResponse(payload)` and `GEM.getAccessToken()`. Reusable for homework grading.
