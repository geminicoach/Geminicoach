...
- **Server-side hardening (DONE 2026-06-23):** exam grading + certificate issuance are now SERVER-SIDE and tamper-proof. Migration `lock_ce_tables_no_client_writes` dropped the client INSERT policies on `certificates` and `exam_attempts` (clients can only SELECT their own rows). New Edge Function **`exam`** (verify_jwt on) holds the answer key server-side: GET `?slug=` returns a drawn, shuffled question set WITHOUT answers; POST `{slug,answers}` grades against the server key, enforces a minimum answered count (draw_mc), records the attempt, and upserts the certificate — all with the service role. `auth.js` exposes `GEM.getExam()`/`GEM.submitExam()`. `exam.html` was rewritten to fetch answerless questions and submit; it no longer holds answers or issues certs. The completion gate stays a soft client UX gate — the **server-graded exam is the integrity anchor** for the cert (defensible for CE). Answer keys live in the `exam` function's `EXAMS` map — update there when content finalizes (the public `exams/<slug>.js` file is now UNUSED by the client; move its answers out of the deploy before real content ships).
